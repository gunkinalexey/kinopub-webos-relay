...
# Первый запуск этой версии скрипта на данной установке никогда не застанет
# APPLIED_AT_FILE существующим - он появляется только когда do_update() сам
# что-то применяет, а это ощутимо реже, чем просто "новый updater.sh
# доехал". Без этого «Последнее обновление» висело бы пустым до следующего
# ПОСЛЕДУЮЩЕГО реального обновления, что для новой установки означает
# «никогда, пока не выйдет что-то ещё». Сеется один раз датой текущего
# коммита - честная оценка (для авто-обновляемой установки деплой обычно и
# правда близок по времени к коммиту), и дальше только настоящие
# применения его двигают.
if [ ! -f "$APPLIED_AT_FILE" ]; then
  git log -1 --format=%ct 2>/dev/null > "$APPLIED_AT_FILE" || date +%s > "$APPLIED_AT_FILE"
fi
