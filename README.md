...
## 2.5. Автообновление (необязательно)

> **Только Linux-хост.** `updater.sh` — bash-скрипт под systemd-таймером, а
> пути в нём (`/opt/kinopub`) линуксовые. На **Windows с Docker Desktop**
> (см. раздел 1) этого не завести напрямую: WSL2 есть, но systemd в нём
> включён не всегда и не гарантирован, а таймер и юниты — часть systemd, не
> Docker. Для локального запуска на Windows блок «Обновление» в Настройках
> просто покажет, что автообновление не настроено, и это не ошибка —
> `GET /update/status` отвечает так же, если каталога `update/` нет. Раздел
> ниже — про Proxmox/любой Linux-сервер, где приложение развёрнуто на
> постоянной основе.

В Настройках появится блок «Обновление»: текущая версия, кнопка «Проверить
обновления», и если новая версия есть — «Обновить». Всё остальное делается
само.

Обновлением занимается скрипт **на хосте, вне Docker**. Причина простая:
пересборка убивает контейнер, из которого её запустили, поэтому изнутри себя
приложение обновиться не может. Второй путь — пробросить в контейнер
`docker.sock` — отвергнут: это равносильно выдаче root на хосте сервису,
который проксирует контент из интернета.

Приложение и скрипт обмениваются двумя файлами в каталоге `update/`: скрипт
пишет туда `status.json`, приложение создаёт `requested`, когда вы нажали
кнопку. Никаких новых привилегий у контейнера не появляется.

### Применяется только необходимое

Скрипт смотрит, какие файлы изменились между коммитами, и делает минимум:

| Что менялось | Что происходит |
|---|---|
| `frontend/` | **Ничего** — смонтирован томом, подхватывается сам |
| `README`, `CHANGELOG`, `deploy/` | Ничего |
| `docker-compose.yml`, `nginx.conf` | Пересоздание контейнеров |
| `backend/` | Пересборка образа, ~30–60 секунд недоступности |

На практике это означает, что большинство обновлений проходит мгновенно и
без перерыва: из последних двенадцати коммитов проекта пересборка
потребовалась бы только одному.

### Установка

Скрипт лежит в репозитории, нужно лишь сделать его исполняемым и включить
таймер. В контейнере kinopub:

```bash
chmod +x /opt/kinopub/deploy/updater.sh
```

```bash
cp /opt/kinopub/deploy/kinopub-updater.service /etc/systemd/system/
```

```bash
cp /opt/kinopub/deploy/kinopub-updater.timer /etc/systemd/system/
```

Если развернули не в `/opt/kinopub` или ветка не `main` — поправьте
`Environment=` в `.service`.
...
```bash
systemctl enable --now kinopub-updater.timer
```

Проверить, что таймер встал:

```bash
systemctl list-timers kinopub-updater.timer
```

Запустить проверку немедленно, не дожидаясь таймера:

```bash
systemctl start kinopub-updater.service
```

```bash
cat /opt/kinopub/update/status.json
```

Таймер проверяет наличие обновлений раз в 5 минут — это только `git fetch` и
сравнение хешей. Само обновление начинается лишь после нажатия кнопки.

### Если обновление не удалось

Скрипт сам откатится на предыдущий коммит: он проверяет `/bridge/health`
после пересборки и, не дождавшись ответа, возвращает прежнюю версию и
пересобирает её обратно. В интерфейсе это будет видно красной строкой.

Журнал:

```bash
journalctl -u kinopub-updater.service -n 50
```
