...
// Just the resolution number - no codec/format/HDR suffix. The picker used to
// read "2160p HDR · HEVC · HLS", which forced the select (and the row of
// player controls around it) wide enough to fit the longest entry even while
// showing the shortest one.
function streamQualityLabel(st,index){var digits=String(st.quality||st.height||'').match(/\d+/);return digits?digits[0]:'Вариант '+(index+1);}
...
// Which episode follows the one playing. detailsMediaList() already flattens
// seasons into the order they are watched in, so "next" is just the entry after
// this one - no season arithmetic, and it steps across a season boundary for
// free. A movie (or an item that arrived without its episode list, e.g. from a
// history card) yields nothing, which is what disables the button.
function playerEpisodeList(){
 var item=state.current;
 if(!item||item.live)return [];
 var list=detailsMediaList(item)||[];
 return list.length>1?list:[];
}
function currentEpisodeIndex(list){
 var id=String(state.episodeId||'');
 if(!id)return -1;
 for(var i=0;i<list.length;i++)if(String(list[i].media_id||list[i].id||'')===id)return i;
 return -1;
}
function nextEpisodeEntry(){
 var list=playerEpisodeList(),i=currentEpisodeIndex(list);
 return (i>=0&&i+1<list.length)?list[i+1]:null;
}
// Disabled rather than hidden on the last episode: a control that vanishes
// makes the remote's focus jump somewhere unexpected mid-playback, and the
// greyed-out button also answers "is there a next one?" without being pressed.
function updateNextEpisodeButton(){
 var btn=$('nextEpisode');if(!btn)return;
 var next=nextEpisodeEntry();
 btn.disabled=!next;
...
//
// Guarded by the same nextEpisodeEntry() the button uses, so the last episode
// of a series simply stops, and a movie or a live channel never qualifies.
function autoplayNextEpisode(){
 if(!state.settings||!state.settings.autoplay_next)return false;
 var next=nextEpisodeEntry();
 if(!next)return false;
 // Position is saved by the caller before this runs; starting the next episode
 // from 0 is deliberate - "continue" is for coming back, not for a title the
 // viewer has never opened.
 $('playerError').textContent='Следующая серия: '+(next.title||'');
...
// Второй источник (локальный /history) убран вместе с таблицей
// watch_progress: он обходил ту же историю KinoPub, что и
// /watching/statuses, и добавлял лишний запрос ради тех же данных.
function loadWatchedStatuses(){return KPApi.watchingStatuses().catch(function(){return {statuses:{}};}).then(function(result){var remote=(result&&result.statuses)||{};state.watchedMap={};for(var id in remote)if(remote.hasOwnProperty(id))state.watchedMap[String(id)]=parseInt(remote[id],10);if(state.appInitialized&&state.route!=='settings')renderCatalog();return state.watchedMap;});}
function initializeAuthenticatedApp(){state.authenticated=true;setAuthLocked(false);$('authModal').classList.add('hidden');
 ...
 // Whether the backend image has FFmpeg at all (WITH_FFMPEG in .env).
 // Only affects the last rung of the audio ladder; left null on failure
...
// picking one writes it straight through and takes effect on the next stream
// URL KinoPub issues.
function serverName(id){for(var i=0;i<state.serverOptions.length;i++)if(state.serverOptions[i].id===id)return state.serverOptions[i].name;return String(id||'');}
function renderServerOptions(selected){var sel=$('setServer');if(!sel)return;var html='';for(var i=0;i<state.serverOptions.length;i++){var o=state.serverOptions[i];html+='<option value="'+o.id+'"'+(o.id===selected?' selected':'')+'>'+esc(o.name)+'</option>';}sel.innerHTML=html||'<option value="">Сервера недоступны</option>';sel.disabled=!state.serverOptions.length;}
function renderServerResults(data){var box=$('serverResults');if(!box)return;if(!data||!data.results||!data.results.length){box.innerHTML='';return;}var html='';for(var i=0;i<data.results.length;i++){var r=data.results[i],cls='server-line';if(r.selected)cls+=' best';if(!r.available)cls+=' down';var stat=r.available
  ?'<span class="server-stat">'+(r.mbps!==null&&r.mbps!==undefined?Number(r.mbps).toFixed(1)+' Мбит/с':'—')+'</span><span class="server-stat">отклик '+(r.latency_ms!==null&&r.latency_ms!==undefined?Math.round(r.latency_ms)+' мс':'—')+'</span>'
  :'<span class="server-stat">'+esc(r.error||'недоступен')+'</span>';
 html+='<div class="'+cls+'"><span class="server-name">'+esc(r.name)+'</span>'+stat+(r.selected?'<span class="server-badge">выбран</span>':'')+'</div>';}
 var when=data.measured_at?new Date(data.measured_at*1000).toLocaleTimeString('ru-RU'):'';
 html+='<div class="server-note">'+(data.applied_best?'Автоматически выбран самый быстрый':'Выбор оставлен прежним')+(when?' · проверено в '+esc(when):'')+'</div>';
 box.innerHTML=html;}
function loadServers(){return KPApi.servers().then(function(data){state.serverOptions=(data&&data.options)||[];state.serverSelected=data&&data.selected;renderServerOptions(state.serverSelected);if(data&&data.last_measured)renderServerResults(data.last_measured);return data;}).catch(function(){state.serverOptions=[];renderServerOptions(null);var sel=$('setServer');if(sel){sel.innerHTML='<option value="">Не удалось получить список</option>';sel.disabled=true;}});}
function applyServerChoice(id){var sel=$('setServer');if(sel)sel.disabled=true;$('settingsStatus').textContent='Переключаем сервер…';return KPApi.selectServer(id).then(function(data){state.serverSelected=data&&data.selected;if(data&&data.options)state.serverOptions=data.options;renderServerOptions(state.serverSelected);$('settingsStatus').textContent='Сервер: '+serverName(state.serverSelected);}).catch(function(err){$('settingsStatus').textContent='Не удалось переключить сервер: '+(err&&err.message?err.message:err);renderServerOptions(state.serverSelected);}).then(function(){if(sel)sel.disabled=!state.serverOptions.length;});}
// Takes several seconds by construction (it switches region, pulls a few MB
// per server, then restores/applies), so the button locks itself and says so
// rather than looking dead.
function measureServers(){var btn=$('measureServers'),sel=$('setServer');if(!btn||btn.disabled)return;btn.disabled=true;if(sel)sel.disabled=true;var label=btn.textContent;btn.textContent='Проверяем…';$('serverResults').innerHTML='<div class="server-note">Проверяем серверы: переключаемся и качаем пробный отрезок с каждого…</div>';
 KPApi.measureServers(true).then(function(data){state.serverSelected=data&&data.selected;for(var i=0;i<state.serverOptions.length;i++)state.serverOptions[i].selected=state.serverOptions[i].id===state.serverSelected;renderServerOptions(state.serverSelected);renderServerResults(data);}).catch(function(err){$('serverResults').innerHTML='<div class="server-note">Не удалось проверить: '+esc(err&&err.message?err.message:String(err))+'</div>';}).then(function(){btn.disabled=false;btn.textContent=label;if(sel)sel.disabled=!state.serverOptions.length;});}
...
function loadSettings(){KPApi.settings().then(function(s){state.settings=s;var icon=normalizeAppIcon(s.app_icon||'kinopub');state.settings.app_icon=icon;$('setQuality').value=s.quality;$('setMode').value=s.stream_mode;$('setAudio').value=s.audio_language;$('setSubs').value=s.subtitles;applySubtitleSize(s.subtitle_size);$('setFullscreen').value=playerFullscreenMode();$('setDeviceProfile').value=capabilityProfile();$('setAutoplay').checked=!!s.autoplay_next;$('setAutoUpdate').checked=!!s.auto_update;$('setAutoUpdateAt').value=s.auto_update_at||'04:00';$('setMotion').checked=!!s.reduce_motion;$('setHistoryFrames').checked=s.history_episode_frames!==false;var radios=document.querySelectorAll('input[name="appIcon"]');for(var i=0;i<radios.length;i++)radios[i].checked=radios[i].value===icon;applyBranding(icon);document.body.classList.toggle('reduce-motion',!!s.reduce_motion);});}
