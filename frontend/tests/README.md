...
| `panel.js` | Full details-screen render: title/rating/vote/tab/info-table structure, season-switching replaces (not appends) the episode strip, screen-not-overlay navigation (Back returns to wherever it was opened from), the series-vs-movie duration split (average/total for a series, first-entry-only for a movie so alternate versions like a film's 24fps/48fps cuts are never summed), the "Похожие" block (hidden on an empty answer, one card per title, late answers discarded), the genre/country/year/director/cast badges (id-backed link vs plain-text fallback, correct filter/search params per click, id-less genre stays text) |
