(function (global) {
  ...
  global.KPApi = {
    ...
    catalog:function(section,feed,page,nonce,perpage){return request('/catalog/list?section='+encodeURIComponent(section||'movie')+'&feed='+encodeURIComponent(feed||'fresh')+'&page='+encodeURIComponent(page||0)+'&perpage='+encodeURIComponent(perpage||48)+'&_='+encodeURIComponent(nonce||Date.now()));}, pageCount:function(section,feed,perpage,refresh){return request('/catalog/page-count?section='+encodeURIComponent(section||'movie')+'&feed='+encodeURIComponent(feed||'fresh')+'&perpage='+encodeURIComponent(perpage||48)+(refresh?'&refresh=true':''));}, item:function(id){return request('/catalog/items/'+encodeURIComponent(id));}, play:function(id,mediaId){return request('/catalog/items/'+encodeURIComponent(id)+'/play'+(mediaId?'?media_id='+encodeURIComponent(mediaId):''));}, search:function(q,mode){return request('/catalog/search?q='+encodeURIComponent(q||'')+'&mode='+encodeURIComponent(mode||'all')).catch(function(){return request('/mock/search?q='+encodeURIComponent(q||''));});}, autocomplete:function(q){return request('/catalog/autocomplete?q='+encodeURIComponent(q||''));},
    kinoHistory:function(page,type){return request('/catalog/history?page='+encodeURIComponent(page||0)+'&perpage=48'+(type?'&type='+encodeURIComponent(type):''));},
    settings:function(){return request('/settings');}, saveSettings:function(v){return request('/settings',{method:'PUT',body:v});}, history:function(mediaId){return request('/history'+(mediaId?'?media_id='+encodeURIComponent(mediaId):''));}, watchingStatuses:function(){return request('/watching/statuses');}, saveProgress:function(v){return request('/history',{method:'PUT',body:v});},
    imageProxyUrl:function(url,nonce,width,height,quality,fallback){var q=BASE+'/image?url='+encodeURIComponent(url);if(width)q+='&width='+encodeURIComponent(width);if(height)q+='&height='+encodeURIComponent(height);if(quality)q+='&quality='+encodeURIComponent(quality);if(fallback)q+='&fallback='+encodeURIComponent(fallback);if(nonce)q+='&v='+encodeURIComponent(nonce);return q;}, streamProxyUrl:function(url){return BASE+'/stream?url='+encodeURIComponent(url);}, subtitleProxyUrl:function(url,offset){var q=BASE+'/subtitle?url='+encodeURIComponent(url),n=Number(offset);if(n)q+='&offset='+encodeURIComponent(n);return q;}, hlsProxyUrl:function(url){return BASE+'/hls?url='+encodeURIComponent(url);},
