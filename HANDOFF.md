...
reflects what's actually deployed. **Currently: backend 0.9.99.**
...
- Backend running version: **0.9.99**. `curl http://localhost:8080/bridge/health`
  (local) or `curl http://kinopub.lan/bridge/health` (deployed) to check.
- **312 frontend checks, 0 failures.** Backend smoke tests also green (see
  Testing below).
